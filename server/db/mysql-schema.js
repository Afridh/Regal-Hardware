...
//   node db/mysql-schema.js <exportDir> [--apply]
import fs from 'node:fs';
import path from 'node:path';

const dir = process.argv[2];
if (!dir) { console.error('usage: node db/mysql-schema.js <exportDir> [--apply]'); process.exit(1) }
...
const tables = Object.keys(man.tables).sort();
...
for (const i of man.indexes || []) {
  ...
  if (!man.tables[i.tablename]) continue;
